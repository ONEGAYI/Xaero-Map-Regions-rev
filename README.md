...
开发环境的 `runClient` 使用 `xaero_world_map_runtime_version` 指定的 Xaero's World Map，并会通过 Gradle 自动下载运行期模组；发布 jar 不会打包 Xaero's World Map。NeoForge 1.21.1 开发运行暂未自动加载 IMBlocker，如遇非输入框快捷键被中文输入法拦截，请先手动切换到英文输入法。
