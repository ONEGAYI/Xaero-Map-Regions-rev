...
开发环境的 `runClient` 使用 `gradle.properties` 中的 `xaero_world_map_runtime_version` 和 `xaero_world_map_runtime_file_id` 指定 Xaero's World Map，并会通过 Gradle 自动同步到 `run-client/mods`；发布 jar 不会打包 Xaero's World Map。NeoForge 1.21.1 开发运行暂未自动加载 IMBlocker，如遇非输入框快捷键被中文输入法拦截，请先手动切换到英文输入法。
