...
- Xaero World Map 1.20.1 开发运行期 jar 内存在 `WorldMap.mapElementRenderHandler.add(...)`、`MapElementRenderer`、`MapElementRenderProvider`、`ElementReader` 等 public 类，可作为后续接入 Xaero 地图元素渲染系统的候选路径；具体运行期版本见 `gradle.properties` 的 `xaero_world_map_runtime_version`。
...
│   ├── network/
│   │   区域同步网络包、地图内编辑 C2S 请求与 S2C 结果回执处理、刷新冷却与广播入口。
│   ├── platform/
│   │   Forge 权限适配。
│   ├── region/
...
│       区域读写服务层与跨维度快照聚合。
├── src/main/resources/
│   Forge 模组元数据、资源包描述、中英文客户端文本资源与地图编辑器 UI 贴图资源。
