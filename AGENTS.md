...
- 本地烟测使用 `./gradlew clean test build`、`./gradlew runClient` 和必要时的 `./gradlew runServer`。ModDevGradle `runClient` 开发烟测通过 `clientAdditionalRuntimeClasspath` 自动加载 Xaero World Map，避免将发布版 raw jar 直接放入 `run/mods/`。`run/` 与 `libs/runtime/` 均不进入 Git。
...
- `.\gradlew.bat runClient`：启动 NeoForge 开发客户端；会通过 `clientAdditionalRuntimeClasspath` 自动下载并加载 Xaero World Map。
...
- Xaero World Map 开发运行期 jar 内存在 `WorldMap.mapElementRenderHandler.add(...)`、`MapElementRenderer`、`MapElementRenderProvider`、`ElementReader` 等 public 类，可作为后续接入 Xaero 地图元素渲染系统的候选路径；具体运行期版本见 `gradle.properties` 的 `xaero_world_map_runtime_version`。
...
│   ├── client/
│   │   客户端区域/颜色历史缓存、玩家收藏色配置、可改快捷键、分页区域管理器、调色盘、地图内编辑状态、编辑提交等待与服务端回执处理、凹多边形填充与 Xaero World Map 覆盖层交互适配。
│   ├── command/
│   │   `/region` 服务端命令入口与权限校验。
│   ├── data/
