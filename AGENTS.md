...
- 主要语言：Java。
- Java 版本：Java 21。
- Xaero's World Map 作为开发运行期客户端依赖使用，不打包进本项目产物；`runClient` 通过 Maven 仓库自动下载外部模组。
...
- 本地烟测使用 `./gradlew clean test build`、`./gradlew runClient` 和必要时的 `./gradlew runServer`。ModDevGradle `runClient` 开发烟测通过 `syncClientRuntimeMods` 自动把 Xaero World Map 同步到独立的 `run-client/mods`，发布 jar 不打包外部模组。`run/`、`run-client/`、`run-server/` 与 `libs/runtime/` 均不进入 Git。
- 使用 Computer Use 进入游戏后如遇到按键无反应，优先手动切换输入法到英文；NeoForge 1.21.1 开发运行暂未自动加载 IMBlocker，后续若补齐兼容运行期依赖再恢复自动加载。
...
- `.\gradlew.bat runClient`：启动 NeoForge 开发客户端；会通过 `syncClientRuntimeMods` 自动下载并加载 Xaero World Map。
...
- Xaero World Map 开发运行期 jar 内存在 `WorldMap.mapElementRenderHandler.add(...)`、`MapElementRenderer`、`MapElementRenderProvider`、`ElementReader` 等 public 类，可作为后续接入 Xaero 地图元素渲染系统的候选路径；具体运行期版本与 CurseForge 文件 ID 见 `gradle.properties` 的 `xaero_world_map_runtime_version` 和 `xaero_world_map_runtime_file_id`。
...
│   ├── client/
│   │   客户端区域/颜色历史缓存、clientbound payload 桥接、玩家收藏色配置、可改快捷键、分页区域管理器、调色盘、地图内编辑状态、编辑提交等待与服务端回执处理、凹多边形填充与 Xaero World Map 覆盖层交互适配。
│   ├── command/
│   │   `/region` 服务端命令入口与权限校验。
│   ├── data/
