...
public final class RegionEditSession {
    ...
    public boolean advanceSelection(List<Region> hitStack, int clickX, int clickZ) {
        ...
        if (selectionIndex >= 0 && new HashSet<>(reversedStack).equals(new HashSet<>(candidateStack))) {
            // 同一组重叠区域 → 推进索引（基于已有 candidateStack 快照）
...
Expected: PASS — 包括 RegionSelectionTest、RegionEditSessionSelectionTest 和所有已有测试。
...
**说明：** neoforge 和 forge 的 `ActionRouter.handleMouse` 代码完全一致，以下改动两个文件都需执行。
...
**Interfaces:**
- Consumes: `RegionEditSession.selectionInfo()`（Task 2）、`SelectionHudText.of(...)`（Task 3）、`ClientRegionCache.regions()`、`Region.label()`

**说明：** 两个平台的 `XaeroMapOverlayController` 结构一致，以下改动两个文件都需执行。注意 neoforge 的 `Rect` 引用路径用 `ResourceLocation.fromNamespaceAndPath`，而 forge 用 `new ResourceLocation`——但本 Task 只涉及 `renderEditor` 和新增的 `renderSelectionHud`，不触碰 ResourceLocation。

- [ ] **Step 1: 修改 neoforge renderEditor 方法，新增 renderSelectionHud 调用**
...
- [ ] **Step 2: 在 neoforge XaeroMapOverlayController 新增 renderSelectionHud 方法**

在 `neoforge/.../XaeroMapOverlayController.java` 中，紧接 `renderEditor` 方法之后（第 130 行 `}` 之后），新增以下方法。注意需要确保文件顶部已 import `net.minecraft.client.gui.Font`（如果尚未 import 则添加）：
...
同时在文件顶部 import 区添加（如果尚无）：

```java
import com.suian.xaeroregionsrev.client.editor.SelectionHudText;
```

- [ ] **Step 3: 对 forge 执行相同的 renderEditor 改动**

在 `forge/mc-1.20.1/.../XaeroMapOverlayController.java` 中执行与 Step 1 完全相同的 `renderEditor` 替换。

- [ ] **Step 4: 对 forge 新增相同的 renderSelectionHud 方法**

在 `forge/mc-1.20.1/.../XaeroMapOverlayController.java` 中执行与 Step 2 完全相同的方法新增和 import 添加。

- [ ] **Step 5: 运行 neoforge 测试确认编译通过**

Run: `./gradlew :neoforge:mc-1.21.1:test`
Expected: PASS — 编译通过。

- [ ] **Step 6: 运行 forge 测试确认编译通过**
...
```bash
git add neoforge/mc-1.21.1/src/main/java/com/suian/xaeroregionsrev/client/xaero/XaeroMapOverlayController.java forge/mc-1.20.1/src/main/java/com/suian/xaeroregionsrev/client/xaero/XaeroMapOverlayController.java
